...
export function MobileBottomDock() {
  ...
  return (
    <div className="sm:hidden fixed bottom-0 left-0 right-0 z-[100] px-3 pt-2 pb-[max(0.6rem,calc(env(safe-area-inset-bottom,0px)+4px))] pointer-events-none">
      {/* Apple Dynamic Island Floating Glass Bar */}
      <div className="w-full max-w-[400px] mx-auto pointer-events-auto bg-[#072448]/60 backdrop-blur-[40px] border border-white/20 rounded-[26px] p-1.5 shadow-[0_16px_40px_rgba(0,0,0,0.35),0_0_0_1px_rgba(255,255,255,0.08)] flex items-center justify-between gap-1 text-white overflow-hidden relative">
        ...
        {/* Apple Health Reports LIS Modal Button */}
        <ReportDownloadModal trigger={
          <button
            className="flex-1 flex items-center justify-center gap-1 min-[360px]:gap-1.5 bg-white/10 hover:bg-white/15 active:bg-white/5 border border-white/10 rounded-[20px] h-12 px-1 min-[360px]:px-2 text-xs font-semibold text-slate-100 transition-all duration-150 active:scale-[0.95] select-none"
            aria-label="Download LIS Lab Report"
          >
            ...
            <div className="flex flex-col text-left leading-tight">
              <span className="text-[10px] font-bold text-white whitespace-nowrap">Reports</span>
            </div>
          </button>
        } />

        {/* Primary CTA: Apple Care Book Home Sample */}
        <button
          onClick={scrollToHomeCollection}
          className="flex-[1.2] flex items-center justify-center gap-1 min-[360px]:gap-1.5 bg-white text-[#072448] rounded-[20px] h-12 px-2 min-[360px]:px-3 text-xs font-bold shadow-[0_4px_12px_rgba(255,255,255,0.2)] transition-all duration-150 hover:bg-white/90 active:scale-[0.95] select-none border border-white"
        ...
        <button
          ...
          className="w-12 h-12 flex-shrink-0 flex items-center justify-center bg-[#25D366] hover:bg-[#20bd5a] rounded-[20px] transition-all duration-150 active:scale-[0.95] shadow-xs select-none border border-[#25D366]"
