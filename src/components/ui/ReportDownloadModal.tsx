    ...
    <Dialog open={open} onOpenChange={handleOpen}>
      <DialogTrigger asChild>
        {trigger || (
          ...
          >
            <div className="w-6 h-6 rounded-lg bg-[#155E9A]/50 flex items-center justify-center text-blue-200 flex-shrink-0">
              <FileDown className="w-3.5 h-3.5" />
            </div>
            <div className="flex flex-col text-left leading-tight">
              <span className="text-[8px] text-blue-200 font-bold uppercase tracking-wider">Reports</span>
      ...
      <DialogContent className="sm:max-w-[420px] p-0 overflow-hidden bg-white/95 backdrop-blur-2xl border border-white/60 shadow-[0_32px_80px_rgba(0,0,0,0.2)] rounded-[32px]">
        ...
        <div className="bg-[#102A43] p-6 text-white text-center relative overflow-hidden">
          {/* Liquid Header Glow */}
          <div className="absolute inset-0 pointer-events-none opacity-40">
            <div className="absolute top-0 right-0 w-[150px] h-[150px] bg-[#C62828] rounded-full blur-[40px]  mix-blend-screen" />
          </div>
          ...
          <DialogDescription className="text-blue-100/90 text-xs leading-relaxed max-w-xs mx-auto relative z-10">
        ...
        <form onSubmit={handleDownload} className="p-5 sm:p-6 space-y-4">
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              >
                <Sparkles className="w-3 h-3 text-[#7A4B2A]" /> Development demo
              </button>}
            </div>
            <Input 
              id="patientId"
          ...
          <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-500 font-medium">
            <span className="flex items-center gap-1">
              <Lock className="w-3 h-3 text-green-700" /> Secure lookup
            </span>
            <span className="flex items-center gap-1">
